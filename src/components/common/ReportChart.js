...
export default function ReportChart({ data }) {
  ...
  return <>
    ...
    >
      <option style={{ display: 'none' }} value="">Filter Data</option>
    ...
    <ResponsiveContainer width="95%" height={260}>
      ...
      <BarChart width={150} height={150} data={data}>
        ...
        <Tooltip />
        <Legend />
        {['', 'totalDana'].includes(dataShown) && <Bar name="Total Dana Dialokasikan" dataKey="totalDana" fill="#0088FE" />}
        {['', 'danaAwal'].includes(dataShown) && <Bar name="Dana Awal" dataKey="danaAwal" fill="#00C49F" />}
        {['', 'danaAkhir'].includes(dataShown) && <Bar name="Dana Akhir" dataKey="danaAkhir" fill="#FFBB28" />}
      </BarChart>
    </ResponsiveContainer>
  </>;
}
